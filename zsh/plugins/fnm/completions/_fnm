...
'--node-dist-mirror=[<https\://nodejs.org/dist/> mirror]:NODE_DIST_MIRROR:_default' \
...
'--arch=[Override the architecture of the installed Node binary. Defaults to arch of fnm binary]:ARCH:_default' \
...
'--resolve-engines=[Resolve \`engines.node\` field in \`package.json\` whenever a \`.node-version\` or \`.nvmrc\` file is not present. This feature is enabled by default. To disable it, provide \`--resolve-engines=false\`.]' \
...
&& ret=0
    ...
    (fnm)
        ...
        case $line[1] in
            (list-remote)
_arguments "${_arguments_options[@]}" : \
'--filter=[Filter versions by a user-defined version or a semver range]:FILTER:_default' \
...
'--use[Use the installed version immediately after installation]' \
'--corepack-enabled[Enable corepack support for each new installation. This will make fnm call \`corepack enable\` on every Node.js installation. For more information about corepack see <https\://nodejs.org/api/corepack.html>]' \
'-h[Print help (see more with '\''--help'\'')]' \
'--help[Print help (see more with '\''--help'\'')]' \
'::version -- A version string. Can be a partial semver or a LTS version name by the format lts/NAME:_default' \
&& ret=0
;;
(i)
_arguments "${_arguments_options[@]}" : \
...
(exec)
_arguments "${_arguments_options[@]}" : \
'--using=[Either an explicit version, or a filename with the version written in it]:VERSION:_default' \
'--node-dist-mirror=[<https\://nodejs.org/dist/> mirror]:NODE_DIST_MIRROR:_default' \
'--fnm-dir=[The root directory of fnm installations]:BASE_DIR:_files' \
'--log-level=[The log level of fnm commands]:LOG_LEVEL:(quiet error info)' \
'--arch=[Override the architecture of the installed Node binary. Defaults to arch of fnm binary]:ARCH:_default' \
...
'-h[Print help (see more with '\''--help'\'')]' \
'--help[Print help (see more with '\''--help'\'')]' \
'*::arguments -- The command to run:_default' \
&& ret=0
;;
(uninstall)
_arguments "${_arguments_options[@]}" : \
...
'i:Install a new Node.js version' \
...
'uni:Uninstall a Node.js version' \
